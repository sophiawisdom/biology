    ...
    if (thresh_bb == 65536){
        for (int i = 0; i < nextMembers/15; i++){
            ...
            for (int j = 0; j < 15; j++) {
                ...
                choiceEntropy >>= 2;
                unsigned short firstIndex = indexEntropy >> 16; // The upper 16 bits are higher quality than the lower 16 bits so use them.
                
                if (choice >= 0 || choice == 0 || choice == 3){ // Both bits from one parent, doesn't matter which
/*#ifdef DEBUG
                    num_first += 1;
#endif*/
...
/*#ifdef DEBUG
                    ...
                    if (firstIndex > thresh_ab){ // if it's a bb, then both alleles are b so result is b
                        ...
                        allele = (firstIndex&2)>>1; //(2-(firstIndex > thresh_aa)); // one less if statement
                    }
        ...
        for (int j = 0; j < nextMembers-(nextMembers/15 * 15); j++){ // This most definitely needs to be unrolled
            ...
            if (choice == 0 || choice == 3){ // Both bits from one parent, doesn't matter which
                ...
                if (firstIndex > thresh_ab){ // if it's a bb, then both alleles are b so result is b
                  ...
                  else if (firstIndex > thresh_aa){ // This means it's an AB, so 50% chance of A or B
                  allele = (firstIndex&2)>>1; // Can't be &1 because the first bit is guaranteed to be 0
                  } // If it's an AA, then it's a guaranteed zero*/
                ...
                if (secondIndex > thresh_ab){ // We know second one is BB
                    ...
                    allele += secondIndex&2; // This used to be firstIndex. This caused an insidious bug where aa and bb were relatively favored 10:12:10 when they should be 8:16:8
                    counts[allele] += 1;
                }
...
/*    int *i_g = initialize_generation(num_organisms);
    ...
    for (int i = 0; i < num_generations; i++){
        printf("Thresh_aa: %d. Thresh_ab: %d. Thresh_bb: %d\n",thresh_aa,thresh_ab,thresh_bb);
...
#pragma clang loop unroll(full)
        ...
        gettimeofday(&end,NULL);
        printf("Took %ld microseconds to progress 100 generations\n",(1000000 * end.tv_sec + end.tv_usec) -  (1000000 * start.tv_sec + start.tv_usec));
    }*/
    return 0;
}
